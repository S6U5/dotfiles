...
## カスタムスラッシュコマンドの雛形(`commands/`)

`commands/` 以下は、そのままコピーして使えるカスタムスラッシュコマンドの例集
(`/document` `/explain` `/fix` `/migrate` `/pr` `/refactor` `/review` `/test`)。
個人用として使うなら `~/.claude/commands/` へ、プロジェクト単位で共有するなら
プロジェクトの `.claude/commands/` へコピーする。

```sh
cp templates/claude/commands/*.md ~/.claude/commands/
```

いずれも `$ARGUMENTS` を使った汎用的な開発ワークフロー用プロンプトで、
個人情報や特定の文脈が分かる内容は含まれていない(コピー先で自分用にカスタマイズしてよい)。
...
- **実験的フラグ(`env` の `CLAUDE_CODE_EXPERIMENTAL_*` 等)** — 将来のバージョンで廃止・改名されうる。`env` に入れてよいのは公式ドキュメント記載の安定した変数のみ(例: `CLAUDE_CODE_DISABLE_FEEDBACK_SURVEY` はテンプレートに設定済み)
