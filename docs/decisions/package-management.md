...
- (2026-08: dotfiles 配布(`home/`)を home-manager の `home.file`(`mkOutOfStoreSymlink`)に統合): 「dotfiles 配布は `home/` の手動シンボリックリンクに任せ、Nix はパッケージのみ」という本 ADR の原則を撤回し、パッケージ導入・dotfiles配布ともに Nix + home-manager に一本化した。過去の `programs.zsh` 統合(上記2エントリ)とは異なり、home-managerの設定生成ロジックには依存せず `home/` の実体ファイルへ直接シンボリックリンクする方式のため、同じ失敗は再発しない設計にしている。検討した代替・選定理由・トレードオフの詳細は新設した `docs/decisions/dotfiles-distribution.md` に分離・統合した。
