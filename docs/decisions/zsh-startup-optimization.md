...
1. **init 出力のファイルキャッシュ**: `eval "$(starship init zsh)"` / `zoxide init zsh` /
   ...
   変わる)にする。同一パスへの上書き更新(apt / brew 等)はバイナリとキャッシュの
   mtime 比較で別途検知する。
...
- (キャッシュ処理の堅牢化): 当初の実装に対して次の4点を修正した。
  (1) 実体パスが変わらない上書き更新(apt / brew 等)を検知できなかったため、
  バイナリとキャッシュの mtime 比較を再生成条件に追加。
  (2) キャッシュへ直接書き込んでいたため、複数シェルの同時起動で書きかけのファイルを
  source しうる競合があり、一時ファイル + mv のアトミック置換に変更。
  (3) キャッシュ先に書き込めない環境(read-only な $HOME 等)で初期化ごと失敗していたため、
  キャッシュなしの直接 eval にフォールバックするよう変更。
  (4) 遅延実行中の `compinit` が compaudit 警告時に y/n プロンプトを出してユーザーの打鍵を
  横取りしうるため `-i`(安全でないディレクトリを黙って除外)に変更。あわせて zcompile に
  `-U` を付け、コンパイル時シェルのエイリアスが .zwc に焼き込まれるのを防止。
