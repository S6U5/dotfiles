...
- クリップボード貼り付けを明示設定(下表)。WezTerm のデフォルトでは Windows Terminal 流の操作(`Ctrl+V`・右クリック・`Shift+Insert`)がどれもクリップボード貼り付けに割り当たっておらず「コピーはできるのに貼り付けできない」ように見えるため

## よく使うキーバインド(このリポジトリでの追加分)

| キー | 内容 |
|---|---|
| `Ctrl+Shift+V` | クリップボードから貼り付け(WezTerm デフォルトと同じだが明示設定) |
| `Shift+Insert` | クリップボードから貼り付け(デフォルトのプライマリセレクション貼り付けから変更。Windows/macOS ではプライマリセレクションが OS クリップボードと繋がっていないため) |
| 右クリック | 選択中ならコピーして選択解除、選択なしならクリップボードから貼り付け(Windows Terminal と同じ操作感) |
| `Ctrl+Shift+O` | ウィンドウ透過のON/OFF切り替え(壁紙が有効なとき(デフォルト有効)のみ。ONにすると壁紙越しにデスクトップが薄く透ける) |

`Ctrl+V` はあえて割り当てていない(nvim の矩形選択など、ペイン内アプリに届くべきキーを潰すため)。
