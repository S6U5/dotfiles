...
- **exit code を常時表示**(`status` モジュール): 成功は緑の `✓ 0`、失敗は赤の `✗ <code>`(例: `✗ 127`)。数字ごと色分けするため、シンボル文字列内で `$status` を展開する書き方にしている(`style` は成功/失敗で分岐できない仕様のため)
- **実行時間を常時・ms 単位まで表示**(`cmd_duration`): `min_time = 0` + `show_milliseconds = true`。`750ms` → `1s500ms` → `1m35s0ms` のように桁が上がると単位も繰り上がる。ms 非表示だと 1 秒未満がすべて `0s` になるため両方セットで設定している
- **入力記号は形を変えず色だけ変化**(`character`): 成功=緑 / 失敗=赤の `❯`。記号を `✗` 等に変えると「これから打つコマンドが間違っている」ように見えるため、失敗の詳細表示は右側の `status` に任せる
