...
- 青枠(Shell・herdr・CLIアプリの3ボックス)は Nix + Home Manager が導入・設定を管理する範囲を外側から点線で囲んで示している。点線の外(OS・ターミナルエミュレータ)はOS標準の環境をそのまま利用しており、Nixでは管理しない。zsh は**バイナリ**(ログインシェル本体)は点線の外(OS標準、判断根拠は `docs/decisions/login-shell.md`)だが、**設定内容**は home-manager が配布するため点線の内側(Shell ボックス)に含めている。
...
home-manager(`home.file` + `mkOutOfStoreSymlink`)が `home/` 以下のファイルを `$HOME` へどうシンボリックリンクするかを、代表的なファイルの対応関係で示した図。「リポジトリ構成」節のツリーの視覚版。
