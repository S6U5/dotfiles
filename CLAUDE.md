...
- **OS 分岐の2系統の使い分け**: コマンド・関数は「常に定義+実行時チェック」で共通層(`home/.config/shell/` / `home/.local/bin/`)に置く。`os/<os>.sh` は**起動時設定(環境変数・エイリアス・PATH 調整など、その OS でだけ読み込ませたい宣言)専用**とする。現在 os/ が空なのは、まだ OS 固有の起動時設定が無いため(プレースホルダとして維持)。
...
- **この dotfiles が定義する環境変数は `DOTFILES_` プレフィックス**を付ける(例: `DOTFILES_OBSIDIAN_VAULT`)。他ツールとの衝突を避け、出どころを明確にするため。内部用のシェル変数は `_dotfiles_` などアンダースコア始まりにする。
