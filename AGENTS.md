...
- プラグインの登録・導入は **`agent-plugins-setup`(`home/.local/bin/`)の明示実行**で行う(`home-manager switch` では走らせない。プラグインはエージェントの挙動を変えるため、意図しないタイミングで増減しないようにする)。ローカル参照の Marketplace は自動更新が効かないので、`git pull` のあとに実行する。**有効/無効の切り替えと取り残しの削除はしない**(検出して報告するだけ。`nix-duplicate-check` と同じ方針)。
