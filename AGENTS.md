...
- `.claude-plugin/` + `.agents/` + `plugins/` — このリポジトリ自身を Claude Code と Codex CLI の**ローカル Marketplace** として公開するためのプラグイン置き場。全ツールで共有する実体はスキル本体(`plugins/<名前>/skills/<名前>/SKILL.md`)だけで、目録とマニフェストは各ツールの形式でそれぞれ持つ(**Claude Code も Codex もルート直下の標準 `plugin.json` を読まない**ため、`.claude-plugin/plugin.json` と `.codex-plugin/plugin.json` を別に置く。Codex はさらに `skills/` を規約で自動検出しないのでパス明示が要る)。**ファイル数を減らすこと自体は価値ではなく、読み手のいるファイルを削ると機能が黙って落ちる**(**dotfiles が正本**)。`home/` と違い `$HOME` には配置せず、各ツールにこのリポジトリを Marketplace として登録して使う。自作スキルはここに置く(`~/.claude/skills/` への直接配布はしない。Claude Code にしか届かないため)。プラグインは目的単位で分割し、MCP サーバーは形式が非互換なため入れない(手順・方針の詳細は `plugins/README.md`)。**SKILL.md はプロンプトなので、カスタマイズされたプロジェクト情報(実プロジェクト名・パス・業務の事情)が最も混ざりやすい。コミット前に必ず全文を精査する。**
...
- 自作スキルは Nix ではなく**プラグイン機構(ローカル Marketplace)で配る**(判断根拠は `docs/decisions/skill-distribution.md`)。登録・導入は **`agent-plugins-setup`(`home/.local/bin/`)の明示実行**で行う(`home-manager switch` では走らせない。プラグインはエージェントの挙動を変えるため、意図しないタイミングで増減しないようにする)。ローカル参照の Marketplace は自動更新が効かないので、`git pull` のあとに実行する。**有効/無効の切り替えと取り残しの削除はしない**(検出して報告するだけ。`nix-duplicate-check` と同じ方針)。
