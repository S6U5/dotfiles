...
- `.claude-plugin/` + `.agents/` + `plugins/` — このリポジトリ自身を Claude Code と Codex CLI の**ローカル Marketplace** として公開するためのプラグイン置き場。スキル本体(`plugins/<名前>/skills/<名前>/SKILL.md`)は1つだけ持ち、目録とマニフェストを各ツールの形式で用意して両方へ配る(**dotfiles が正本**)。`home/` と違い `$HOME` には配置せず、各ツールにこのリポジトリを Marketplace として登録して使う。プラグインは目的単位で分割し、MCP サーバーは形式が非互換なため入れない(手順・方針の詳細は `plugins/README.md`)。**SKILL.md はプロンプトなので、カスタマイズされたプロジェクト情報(実プロジェクト名・パス・業務の事情)が最も混ざりやすい。コミット前に必ず全文を精査する。**
